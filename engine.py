...
import time
import utils
from memory_tools import GPUMemoryTimeline, dump_cuda_memory_snapshot
...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    original_model.eval()
    timeline = GPUMemoryTimeline(device=device)
    ...
    for it, (input, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        timeline.close_iteration()
        # N iteration마다 메모리 스냅샷 저장
        if getattr(args, "snapshot_every", 0) > 0 and (it + 1) % args.snapshot_every == 0:
            snap_dir = getattr(args, "snapshot_dir", "./mem_snapshots")
            snap_path = dump_cuda_memory_snapshot(out_dir=snap_dir, tag=f"iter{it+1}")
            print(f"[MEM] CUDA memory snapshot saved → {os.path.abspath(snap_path)}")
    ...
    try:
        os.makedirs(getattr(args, "memprof_out", "./mem_profile"), exist_ok=True)
        json_path = timeline.to_json(os.path.join(args.memprof_out, "timeline.json"))
        png_path  = timeline.plot(os.path.join(args.memprof_out, "timeline.png"))
        print("\n[MEM] ===== Memory profiling results =====")
        print(f"[MEM] Timeline JSON  → {os.path.abspath(json_path)}")
        print(f"[MEM] Timeline Plot  → {os.path.abspath(png_path)}")
        print(f"[MEM] Timeline saved: {json_path}")
        if png_path:
            print(f"[MEM] Timeline plot: {png_path}")
    except Exception as e:
        print(f"[MEM] Failed to write timeline: {e}")
