...
class VisionTransformer_dual(nn.Module):
    """ Vision Transformer
    A PyTorch impl of : `An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale`
        - https://arxiv.org/abs/2010.11929
    """
...
class AdaptiveLayerDrop:
    ...
    def update_state(self, global_step):
        self.current_theta = (1. - self.theta) * np.exp(-self.gamma * global_step) + self.theta

    def get_theta(self):
        return self.current_theta

class VisionTransformer_ALD_Dual(nn.Module):
    """ Vision Transformer
    A PyTorch impl of : `An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale`
        - https://arxiv.org/abs/2010.11929
    """
    ...
    def __init__(
            ...
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False,
            use_g_prompt=False, g_prompt_length=None, g_prompt_layer_idx=None, use_prefix_tune_for_g_prompt=False,
            use_e_prompt=False, e_prompt_layer_idx=None, use_prefix_tune_for_e_prompt=False, same_key_value=False, ald=AdaptiveLayerDrop, theta=0.75, gamma=0.001, tau=8, alpha=0.9):
        """
        Args:
            img_size (int, tuple): input image size
            patch_size (int, tuple): patch size
        ...
        self.theta = theta
        self.gamma = gamma
        self.ald = AdaptiveLayerDrop(theta=self.theta, gamma=self.gamma)

        self.tau = tau
        self.alpha = alpha
        self.global_step = 0
        self.total_layers_in_epoch = 0
        self.dropped_layers_in_epoch = 0
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_g_prompt or self.use_e_prompt:
                ...
                if train:
                    # PLD mechanism
                    current_theta = self.ald.get_theta()
                    L = len(self.blocks)  # Total number of layers
                    step = (1 - current_theta) / L
                    p = 1.0  # Initial drop probability for the first layer
                    ...
                    for i, block in enumerate(self.blocks):
                        action = torch.bernoulli(torch.tensor([p]).to(x.device))
                        if action.item() == 1:
                            # Incorporate the regular forward pass here
                            if i in self.g_prompt_layer_idx:
                                if self.use_prefix_tune_for_g_prompt:
                                    g_prompt_counter += 1
                                    idx = torch.tensor([g_prompt_counter] * x.shape[0]).to(x.device)
                            ...
                            executed_layers.append(i+1)
                        else:
                            if i in self.g_prompt_layer_idx or i in self.e_prompt_layer_idx:
                                x = block(x)
                            else:
                                dropped_layers.append(i+1)
                        p -= step 
                    
                        current_theta *= self.alpha
                        self.ald.update_state(current_theta)
                    
                    self.total_layers_in_epoch += L
                    self.dropped_layers_in_epoch += len(dropped_layers)
                else:
                    for i, block in enumerate(self.blocks):
                        if i in self.g_prompt_layer_idx:
                            if self.use_prefix_tune_for_g_prompt:
    ...
    def get_dropped_layers_info(self):
        return {
            'total_layers': self.total_layers_in_epoch,
            'dropped_layers': self.dropped_layers_in_epoch,
        }


class VisionTransformer_ALD(nn.Module):
    ...
    def __init__(
            ...
            weight_init='', embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block,
            prompt_length=None, embedding_key='cls', prompt_init='uniform', prompt_pool=False, prompt_key=False, pool_size=None,
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False, ald=AdaptiveLayerDrop, theta=0.5, gamma=0.001, tau=16, alpha=0.9):
        """
        Args:
            img_size (int, tuple): input image size
            patch_size (int, tuple): patch size
        ...
        self._packed_by_layer = {}
        self._gpu_staging = {} 
        self._staging_stream = torch.cuda.Stream()
        self._current_step = None
        self._current_layer = None
        self.activation_cache = {}  # CPU/디스크에 저장된 activation 관리
        self.cpu_cast_dtype = torch.float16
        self.pin_cpu_tensors = True 
        self.cpu_cache_budget_bytes = None
        ...
        self.theta = theta
        self.gamma = gamma
        self.ald = AdaptiveLayerDrop(theta=self.theta, gamma=self.gamma)  # Adjust these hyperparameters as needed
        self.global_step = 0
        ...
        self.prompt_selection_time = 0.0
        self.total_layers_in_epoch = 0
        self.dropped_layers_in_epoch = 0
    ...
    @contextmanager
    def _layer_ctx(self, step, layer):
        prev_s, prev_l = self._current_step, self._current_layer
        self._current_step, self._current_layer = step, layer
        try:
            yield
        finally:
            self._current_step, self._current_layer = prev_s, prev_l


    def _pack_to_cpu(self, t):
        # CPU로 오프로딩 + 인덱싱
        cpu_t = t.detach().to('cpu', non_blocking=True).pin_memory()
        packed = (t.device, cpu_t, t.dtype, t.shape,
                self._current_step, self._current_layer)
        key = (self._current_step, self._current_layer)
        self._packed_by_layer.setdefault(key, []).append(packed)
        return packed

    def _unpack_to_gpu(self, packed):
        device, cpu_t, dtype, shape, step, layer = packed

        # 미리 다음에 필요해질 레이어 프리페치 (현재 layer 기준 i → i-d)
        target = layer - self.prefetch_distance
        if target >= 0:
            self._schedule_prefetch(step, target)

        # 이후 동일 (staging 있으면 사용, 없으면 즉시 복사)
        key = (step, layer)
        staged_list = self._gpu_staging.get(key)
        if staged_list and len(staged_list) > 0:
            gpu_t = staged_list.pop()
            if not staged_list:
                self._gpu_staging.pop(key, None)
            return gpu_t.view(shape).to(dtype)
        return cpu_t.to(device, non_blocking=True).view(shape).to(dtype)


    def _schedule_prefetch(self, step, target_layer):
        key = (step, target_layer)
        if key not in self._packed_by_layer or key in self._gpu_staging:
            return  # 없거나 이미 staging됨

        packs = self._packed_by_layer[key]
        staged = []

        # 별도 스트림에서 비동기 H2D
        with torch.cuda.stream(self._staging_stream):
            for p in packs:
                device, cpu_t, dtype, shape, _, _ = p
                staged.append(cpu_t.to(device, non_blocking=True))
        self._gpu_staging[key] = staged
        # 나중에 주 스트림이 이 전송과 동기화되도록 이벤트를 기록해도 됨(선택):
        # self._staging_stream.synchronize()  # 강제 동기화는 비추
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if train:
            ...
            # forward_features 반복에서 매 블록 호출을 이렇게 감싸기
            #이 코드는, pytorch autograd(자동 미분 엔진)에서 backward에 쓰려고 저장되는 텐서들의 저장/복원 경로를 가로채는 context manager를 가져오는 import
            #이 context안에서 실행되는 연산들이 ctx.save_for_backward(t)로 무언가를 저장하려고 하면 
            #저장 직전에 pack 콜백이 호출되고, backward에서 그 텐서가 필요해지는 순간 unpack 콜백이 호출됨.
            from torch.autograd.graph import saved_tensors_hooks  

            for i, block in enumerate(self.blocks):
                action = torch.bernoulli(torch.tensor([p]).to(x.device))
                print(f"[ALD DEBUG] Block {i+1}, Drop Probability: {p:.4f}, Action: {'Execute' if action.item() == 1 else 'Drop'}")
                if action.item() == 1:
                    current_step = self.global_step
                    
                    #이 context안에서 수행되는 모든 연산 중, backward를 취해 텐서를 저장하려는 연산은
                    #저장 시 pack_fn(tensor)가 먼저 호출되어 우리가 정의한 방식으로 저장물이 바뀜
                    #복원 시 unpack_fn(packed)가 호출되어 우리가 정의한 방시긍로 텐서를 복원한다
                    with self._layer_ctx(current_step, i):
                        with saved_tensors_hooks(self._pack_to_cpu, self._unpack_to_gpu):
                            x = block(x)
                        #이 한 블록의 forward 전체가 context 내부에서 실행
                        #블록 내부의 모든 연산 중 save_for_backward를 사용하는 연산자(예: matmul, softmax, LayerNorm, Attention 등)
                        #의 저장물만 cpu로 offload됨
                        #블록의 출력 텐서 x 자체는 건드리지 않기 때문에 forward는 정상 진행됨
                    ...
                    dropped_layers.append(i + 1)
                p -= step
                self.ald.update_state(self.global_step)
                self.global_step += 1

            print(f"[ALD DEBUG] # of Executed Layers: {len(executed_layers)}, # of Dropped Layers: {len(dropped_layers)}, Total Layers: {L}")
            self.total_layers_in_epoch += L
            self.dropped_layers_in_epoch += len(dropped_layers)
        else:
            if self.grad_checkpointing and not torch.jit.is_scripting():
        ...
        # loss.backward()가 호출되면 레지스터 훅 실행 -> 수정 필요. gradient 계산 전에 로드해야 되는데 gradient 계산이 끝난 후에 트리거되고 있음.
        '''
        if train:
            x.register_hook(lambda grad: self._backward_hook(grad, executed_layers))
        '''
        return res
    
    def _backward_hook(self, grad, executed_layers): #drop 안된 레이어 activation만 로딩하여 gradient 계산
        """Backward pass에서 필요한 activation만 GPU로 로드"""
        for layer_idx in reversed(sorted(executed_layers)):
            try: #activation을 디스크에서 로딩
                activation = self._load_activation(layer_idx - 1, self.global_step)
                # GPU로 복원
                activation = activation.to(self.device, non_blocking=True)
            except KeyError:
                print(f"[WARNING] Activation for layer {layer_idx-1} at step {self.global_step} not found.")
                continue
           
        # 해당 global_step의 activation 정리
        self._clear_activation(self.global_step)
        return grad
    
    def forward_head(self, res, pre_logits: bool = False): #classification layer 이후의 output(결과 분류)
        x = res['x']
        if self.class_token and self.head_type == 'token':
            x = x[:, 0]
        elif self.head_type == 'gap' and self.global_pool == 'avg':
            ...
            # Extract only the prompted embeddings.
            x = x[:, 1:(1 + self.total_prompt_len)] if self.class_token else x[:, 0:self.total_prompt_len]
            x = x.mean(dim=1)
        elif self.head_type == 'token+prompt' and self.prompt_pool and self.class_token:
            x = x[:, 0:self.total_prompt_len + 1]
        ...
        res['pre_logits'] = x #얘를 저장하면 됨(activation 값)

        x = self.fc_norm(x) #최종 representation에 normalization 적용
        
        res['logits'] = self.head(x) #linear classifier
        
        return res
 
    def forward(self, x, task_id=-1, cls_features=None, train=False): #최종 결과만 포함
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train)
        res = self.forward_head(res)
        return res
...
def _create_vision_transformer_adaptive_layer_dropping(variant, pretrained=False, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')

    pretrained_cfg = resolve_pretrained_cfg(variant, pretrained_cfg=kwargs.pop('pretrained_cfg', None))
    model = build_model_with_cfg(
        VisionTransformer_ALD, variant, pretrained,
        pretrained_cfg=pretrained_cfg,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in pretrained_cfg['url'],
        **kwargs)
...
def _create_vision_transformer_adaptive_layer_dropping_dual(variant, pretrained=False, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')

    pretrained_cfg = resolve_pretrained_cfg(variant, pretrained_cfg=kwargs.pop('pretrained_cfg', None))
    model = build_model_with_cfg(
        VisionTransformer_ALD_Dual, variant, pretrained,
        pretrained_cfg=pretrained_cfg,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in pretrained_cfg['url'],
        **kwargs)
...
@register_model
def vit_large_patch16_224_adaptive_layer_drop(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer_adaptive_layer_dropping('vit_large_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def vit_base_patch16_224_adaptive_layer_drop_dual(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer_adaptive_layer_dropping_dual('vit_base_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def vit_large_patch16_224_adaptive_layer_drop_dual(pretrained=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer_adaptive_layer_dropping_dual('vit_large_patch16_224', pretrained=pretrained, **model_kwargs)
    return model