...
class AdaptiveLayerDrop:
    ...
    def update_state(self, global_step): #current_theta (레이어 보존 확률) update
        #offset = 10  
        #self.current_theta = (1 - self.theta) * np.exp(-self.gamma * (global_step + offset)) + self.theta
        
        self.current_theta = (1. - self.theta) * np.exp(-self.gamma * global_step) + self.theta #아니면 여기서 1-theta 해서 처음 배치에만 drop 되고 돌아오는건가? 
...
class VisionTransformer_ALD(nn.Module):
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if hasattr(self, 'prompt'):
            ...
            res = self.prompt(x, prompt_mask=prompt_mask, cls_features=cls_features)
            self.total_prompt_len = res['total_prompt_len']
            # print(f"prompt norm: {prompt_norm}")
            # print(f"[ALD DEBUG] Total Prompt Length: {self.total_prompt_len}")
            
            x = res['prompted_embedding']
            # print(f"x_embed_norm: {x_embed_norm}")
            # print(f"x_embed_norm_shape: {x_embed_norm.shape}")
            # print(f"[ALD DEBUG] Prompted Embedding Shape: {x.shape}")
        else:
            res=dict()
        if self.cls_token is not None:
            x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
        ...
        if train:
            ...
            # Lists to track executed and dropped layers
            executed_layers = []
            dropped_layers = []

            for i, block in enumerate(self.blocks):
                action = torch.bernoulli(torch.tensor([p])).to(x.device)
                print(f"[DEBUG] Block {i+1}, Drop Prob: {p:.4f}, Action: {'Execute' if action.item() == 1 else 'Drop'}")
                if action.item() == 1:
                    executed_layers.append(i)
                else:
                    ald_dropped.add(i)
                    dropped_layers.append(i)
                    print(f"[ALD DROP] Block {i+1} was dropped by ALD.")
                p -= step
                self.ald.update_state(self.global_step)
                self.global_step += 1

            # Step 2: force drop 대상 집합 생성 (ALD로 drop된 것 제외)
            L = len(self.blocks)
            half = L // 2
            shallow_pool = [i for i in range(0, half) if i not in ald_dropped]
            deep_pool = [i for i in range(half, L) if i not in ald_dropped]

            # 강제 drop 개수 설정
            num_shallow_total = min(1, 1 + self.global_step // self.tau)
            num_deep_total = min(2 + self.global_step // self.tau, 2)

            # 이미 ALD로 drop된 레이어 중 shallow/deep 분리
            ald_shallow = [i for i in ald_dropped if i < half]
            ald_deep = [i for i in ald_dropped if i >= half]

            # 강제 drop이 실제로 더 필요한 수 계산
            num_drop_shallow = max(0, num_shallow_total - len(ald_shallow))
            num_drop_deep = max(0, num_deep_total - len(ald_deep))

            # force drop 샘플링
            drop_shallow = set(random.sample(shallow_pool, min(len(shallow_pool), num_drop_shallow)))
            drop_deep = set(random.sample(deep_pool, min(len(deep_pool), num_drop_deep)))
            force_drops = drop_shallow.union(drop_deep)

            # Step 3: 최종 실행
            final_output = x
            x_tmp = x
            x = x_tmp
            for i in executed_layers:
                if i in force_drops:
                    dropped_layers.append(i)
                    print(f"[FORCE DROP] Block {i+1} was force-dropped.")
                    continue
                x = self.blocks[i](x)
            #한 배치당 출력하는거
            print(f"[DEBUG] Executed: {len(executed_layers) - len(force_drops)}, Dropped: {len(dropped_layers)}, Total: {L}")
            self.total_layers_in_epoch += L
            self.dropped_layers_in_epoch += len(dropped_layers)
