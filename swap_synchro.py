...
class VisionTransformer_ALD(nn.Module):
    ...
    def __init__(
        ...
        self.swap_pack_ms  = 0.0   # 이번 step에서 PACK 총합 (D2H)
        self.swap_unpack_ms = 0.0 
        ...
        # VisionTransformer_ALD.__init__ 또는 모델 생성 직후
        from debug_swap import SwapDebugTracker
        self.swap_dbg = SwapDebugTracker(dev_id=torch.cuda.current_device(), verbose=True)
    ...
    # forward 중 save_for_backward(t)를 부르는 순간 autograd가 이 함수를 호출 (autograd가 중간 값 저장/복원의 주체. autograd는 모든 텐서 연산을 기록하고 자동으로 미분을 계산하는 시스템 엔진)
    def _pack_to_cpu(self, t):
        # saved tensor를 CPU(핀 메모리)로 옮겨 저장
        #t.detach()= 계산 그래프에서 분리시키는 역할 (gradient tracking을 진행하지 않음)
        #to cpu= gpu -> cpu. non-blocking = 비동기적으로 전송을 허용
        #pin memory: cpu memory를 pinn locked로 만들어서 이후 cpu -> gpu 복사가 더 빠르게 이루어지도록 준비.
        cpu_t = t.detach().to('cpu', non_blocking=True).pin_memory()
        
        tag = f"{self._active_tag}#pack{self._pack_count}"
        # --- 디버깅: 장치/핀 확인 + 등록 ---
        assert cpu_t.device.type == 'cpu', "PACK 실패: device가 CPU가 아님"
        assert getattr(cpu_t, "is_pinned", lambda: False)(), "PACK 실패: pinned memory 아님(비동기 H2D 불가)"
        self._pack_count += 1
        # (디버깅) CPU로 잘 갔는지 기록
        if hasattr(self, "swap_dbg"):
            self.swap_dbg.on_pack(cpu_t, tag)

        return (t.device, cpu_t, t.dtype, t.shape, tag)
        #cpu_t(cpu에 저장한 텐서)만 저장하는게 아니라 나중에 복원할 때 필요할 메타데이터와 함께 저장. (gpu 장치, 원래 dtype, 원래 shape )
        #d이 튜플 전체가 packed object로 autograd에 보관됨.

    #saved_tensors_hooks의 unpack 콜백. backward에서 해당 텐서가 필요해지는 순간에 호출
    def _unpack_to_gpu(self, packed):
        device, cpu_t, dtype, shape, tag = packed #pack에서 돌려준 튜플을 해체(원래 장치,데이터,형태 정보를 복원)
        # --- 진짜 복원 ---
        out = cpu_t.to(device, non_blocking=True).view(shape).to(dtype)
        # --- 디버깅: 장치/모양/타입 검증 + 등록 ---
        assert out.device == device, "UNPACK 실패: target device와 다름"
        assert out.shape == shape, "UNPACK 실패: shape mismatch"
        assert out.dtype == dtype, "UNPACK 실패: dtype mismatch"
        
        # (디버깅) 복원된 텐서의 실제 태그로 기록
        if hasattr(self, "swap_dbg"):
            # 보기 좋게 pack→unpack으로 접미사만 바꿔도 됨
            u_tag = tag.replace("#pack", "#unpack")
            self.swap_dbg.on_unpack(out, u_tag)

        # backward 시점에 다시 GPU로 올림
        return cpu_t.to(device, non_blocking=True).view(shape).to(dtype)
        #cpu_t.to(device)= cpu -> gpu 복사. 그리고 뒤에 view랑 dtype은 원래 모양으로 돌리기 위해 쓰는 것.
        ...
        if train:
            ...
            for i, block in enumerate(self.blocks):
                ...
                if action.item() == 1:
                    ...
                    self._active_tag = f"step{self.global_step}_blk{i}"
                    self._pack_count = 0
                    self._unpack_count = 0
